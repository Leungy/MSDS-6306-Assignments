...
ggplot(medianABV, aes(y=median_ABV, x=reorder(State,-median_ABV),fill=State)) + geom_bar(stat = "identity")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))+xlab("State")+ylab("Median ABV")

#Plotmedian IBU
#reorder from highest to lowest
ggplot(medianIBU, aes(y=median_IBU, x=reorder(State,-median_IBU),fill=State)) + geom_bar(stat = "identity")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))+xlab("State")+ylab("Median IBU")
```
...
ggplot(Brewery_Beer, aes(x=IBU, y=ABV))+geom_point()+geom_smooth(method="lm")

#Correlation Matrix of IBU vs ABV
AbvIBU<-Brewery_Beer[,c(4,5)]
ggcorr(AbvIBU,palette = "RdBu", label = TRUE)
```
