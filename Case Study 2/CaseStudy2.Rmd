...
#Correlation plot
ggcorr(dplyr::select_if(HRdata, is.numeric),hjust = 0.95, size = 3,label = TRUE, label_size = 3, label_round = 1, label_alpha = TRUE,layout.exp=15,palette = "PuOr")+ ggplot2::labs(title = "Correlation Coefficients of Numeric HR Data Prior Recoding/Transformation")
...
ggcorr(dplyr::select_if(HRdata, is.numeric),hjust = 0.95, size = 3,label = TRUE, label_size = 3, label_round = 1, label_alpha = TRUE,layout.exp=15,palette = "PuOr")+ ggplot2::labs(title = "Correlation plot of Numeric HR Data with logMonthlyIncome & AttritionRecode ")

```

##Linear Regression for variable ideas
```{r linear-reg-VIF}
...
DistanceFromHome, NumCompaniesWorked and YearsInCurrentRole were common factors in both attrition and salary questions.
...
cl.validate<-as.factor(ifelse(Pred.Validate>=0.5,"Yes","No"))

#write the cl.validate into the validation data set and rename csv for output
```

##Determine Optimal K coefficient, split the data into Train & Test (60/40) and predict logMonthlyIncome using validation data
```{r salary}
#load the attrition data validation set
...
#glm(logMonthlyIncome~DistanceFromHome+NumCompaniesWorked+TotalWorkingYears+YearsAtCompany+YearsInCurrentRole+YearsWithCurrManager)
glm.fit1b
...
#Exponentiate the results to transform back into Predicted MonthlyIncome
Predicted.MonthlyIncome=data.frame(exp(pred.SalaryValidate))
